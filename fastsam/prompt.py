...
#            import clip
    ...
    def retrieve_batch(self, model, preprocess, elements, search_texts: list, device) -> torch.Tensor:
        ...
        :param search_texts: 文本列表（如['Car', 'Van', 'Truck']）
        :param device: 设备（cpu/cuda）
        :return: 相似度得分矩阵 [num_elements, num_texts]，每行是该element对所有text的softmax得分
        """
        # 1. 批量预处理图片
        preprocessed_images = [preprocess(image).to(device) for image in elements]
        stacked_images = torch.stack(preprocessed_images)  # [num_elements, 3, 224, 224]

        # 2. 批量编码图片
        image_features = model.encode_image(stacked_images)  # [num_elements, 512]
        image_features /= image_features.norm(dim=-1, keepdim=True)
        ...
        temp = 0.05
        # 4. 批量计算相似度矩阵 + softmax
        probs = image_features @ text_features.T  # [num_elements, num_texts]
        probs_softmax = (probs / temp).softmax(dim=-1)  # [num_elements, num_texts]
    ...
    # -------------------------- 新增：多类别概率计算方法 --------------------------
    def _normalize_prob(self, prob_map):
        """归一化概率矩阵（逐像素和为1）"""
        eps = 1e-8
        prob_sum = prob_map.sum(axis=-1, keepdims=True) + eps
        return prob_map / prob_sum

    def text_prompt_multi_classes(self, categories):
        ...
        # 2. 对每个类别执行text_prompt，获取mask和CLIP分数
        format_results = self._format_results(self.results[0], 0)
        ...
        clip_model, clip_preprocess = self.clip_model, self.clip_preprocess

        n_crops = len(cropped_boxes)
        
        # prompt_list= [f"a photo of a {cat.lower()}" for cat in categories[1:]]
        clip_scores = self.retrieve_batch(clip_model,
                                          clip_preprocess,
                                          cropped_boxes, 
                                          # prompt_list,
                                          categories[1:], 
        ...
        print(clip_scores)
        clip_scores[clip_scores < CLIP_PROB_THRESH] = 0

        for crop_idx in range(n_crops):
            real_idx = crop_idx + sum(np.array(filter_id) <= crop_idx)
            if real_idx < 0 or real_idx >= len(annotations):
                continue 

            crop_mask = annotations[real_idx]['segmentation']
            if crop_mask.shape != (H, W):
                crop_mask = cv2.resize(crop_mask.astype(np.uint8), (W, H), interpolation=cv2.INTER_NEAREST)
            crop_mask = crop_mask.astype(np.float32)
            crop_mask = np.clip(crop_mask, 0.0, 1.0)
            bool_mask = crop_mask > 0

            if clip_scores[crop_idx].shape[0] != (C-1):
                print("Wrong num CLASSES.")
                continue 
        ...
        if np.any(semantic_mask):
            # 对前景区域的前景类（1~C-1）归一化
            pixel_prob[semantic_mask, 1:] = self._normalize_prob(pixel_prob[semantic_mask, 1:])
        # 非语义区域：背景类（第0通道）设为1，其余为0
        ...
        return class_masks, pixel_prob, class_clip_scores
